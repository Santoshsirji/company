...
const MyForm: React.FC<VacancyFormProps> = ({
     initialData 
}) => {
    ...
    const title = initialData ? "Edit Vacancy" : "Create an Vacancy";
    const description = initialData ? "Edit an Vacancy" : "Add a new Vacancy";

    const toastMessage = initialData ? "Vacancy Updated" : "Vacancy Created";
    ...
    const onSubmit = async (data: VacancyFormValues) => {
        try {
            setLoading(true); 
            if(initialData) {
                await axios.patch(`/api/vacancy/${params.vacancyId}`, data);
    ...
    const onDelete = async () => {
        try {
            setLoading(true);
            await axios.delete(`/api/vacancy/${params.VacancyId}`);
            router.refresh();
            router.push(`/admin/vacancy`);
            router.refresh();
